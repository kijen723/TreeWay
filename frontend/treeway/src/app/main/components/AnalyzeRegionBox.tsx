...
export default function DetailBox({
  ...
  function formatAddress(address: string) {
    // '구'나 '시'까지만 남기기 위한 정규식 패턴
    const guIndex = address.indexOf('구');
    const siIndex = address.indexOf('시', address.indexOf('도') + 1); // '도' 다음에 나오는 '시'를 찾아야 함

    if (guIndex !== -1) {
      return address.slice(0, guIndex + 1); // '구'까지 자름
    } else if (siIndex !== -1) {
      return address.slice(0, siIndex + 1); // '시'까지 자름
    }

    // '구'나 '시'가 없으면 전체 주소를 반환
    return address;
  }
